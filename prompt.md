...
- セッション時間：約51分想定（motor校正7分 + 本課題44分）
- 試行時間目安：1試行あたり約8.75秒（目標値）
- 本課題試行数：100音 × 3試行 = 300試行（rep1-3を全使用, r=0.973 vs 6rep基準）
...
### 4.2 Trial画面（共通）
- ボタンを押し込んでから500ms後に音声を自動再生（リリース入力方式）
- 画面中央に大きな「押下ボタン」を表示
- リリース入力：キーボード（スペースキー離し）/ マウスボタン離し / タッチ終了
- ボタンを離した時点でRT計録・音声停止：
  1. 音声停止（課題音声＋合図音の両方）
  2. 直ちに回答画面へ遷移
...
### 4.5 参加者への教示方針
- **ボタン操作**: ボタンを押し続けながら音声を聴くよう指示する。識別できた瞬間に離すことで反応時間を計測する。
- **予測押し防止**: 「識別が確定する前に予測的にボタンを離さないよう注意してください」と明示する。
- **誤押し**: 誤ってボタンを離してしまった場合は「誤ってボタンを押してしまった」ボタンで後回しにできることを伝える。
...
5. ボタンを押し込んだ500ms後に序歌が再生開始。参加者が「識別できた」と判断した瞬間にボタンを離す（`t_press` 記録）  
6. 音声停止  
